...
def _csv(rows: list[dict]) -> str:
    cols = ["id", "timestamp", "exchange_timestamp", "price", "volume",
            "action", "direction"]
...
class TestFillOnPriceChange:
    def test_taker_create_then_delete_records_fill(self, tmp_path):
        ...
        path.write_text(_csv([
            dict(id=1, timestamp=100, exchange_timestamp=100,
                 price=79107.0, volume=0.5, action="created",
                 direction="bid"),
            dict(id=1, timestamp=101, exchange_timestamp=101,
                 price=78323.0, volume=0.0, action="deleted",
                 direction="bid"),
        ]))
    ...
    def test_aggressor_walking_levels_records_every_fill(self, tmp_path):
        ...
        rows = [
            dict(id=1, timestamp=100, exchange_timestamp=100,
                 price=80000.0, volume=1.0, action="created", direction="bid"),
            dict(id=1, timestamp=100, exchange_timestamp=100,
                 price=79000.0, volume=0.7, action="changed", direction="bid"),
            dict(id=1, timestamp=100, exchange_timestamp=100,
                 price=79000.0, volume=0.5, action="changed", direction="bid"),
            dict(id=1, timestamp=100, exchange_timestamp=100,
                 price=79100.0, volume=0.1, action="changed", direction="bid"),
            dict(id=1, timestamp=101, exchange_timestamp=101,
                 price=79200.0, volume=0.0, action="deleted", direction="bid"),
        ]
