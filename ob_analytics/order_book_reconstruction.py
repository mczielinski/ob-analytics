...
def order_book(events: pd.DataFrame, tp: pd.Timestamp = pd.Timestamp.now(tz='UTC'), 
               ...
               min_bid: float = 0, max_ask: float = np.inf) -> dict:
  """
  ...
  def active_bids(active_orders: pd.DataFrame) -> pd.DataFrame:
    bids = active_orders[(active_orders['direction'] == 'bid') & (active_orders['type'] != 'market')]
  ...
  def active_asks(active_orders: pd.DataFrame) -> pd.DataFrame:
    asks = active_orders[(active_orders['direction'] == 'ask') & (active_orders['type'] != 'market')]
  ...
  created_before = events[(events['action'] == 'created') & (events['timestamp'] <= tp)]['id']
  deleted_before = events[(events['action'] == 'deleted') & (events['timestamp'] <= tp)]['id']
  ...
  changed_orders = changed_orders.sort_values(by='timestamp', ascending=False).groupby('id').head(1)
  active_orders = pd.concat([active_orders[active_orders['action'] != 'changed'], changed_orders])
