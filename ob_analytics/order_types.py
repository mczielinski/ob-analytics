...
def set_order_types(events: pd.DataFrame, trades: pd.DataFrame) -> pd.DataFrame:
  ...
  events['type'] = 'unknown'
  events['type'] = pd.Categorical(events['type'], categories=['unknown', 'flashed-limit', 'resting-limit', 
                                                             'market-limit', 'pacman', 'market'], ordered=True)
  ...
  created_deleted_ids = created[created['id'].isin(deleted['id']) & ~created['id'].isin(changed['id'])]['id'].reset_index(drop=True)
  # Get volumes from the 'deleted' DataFrame with matched IDs and reset the index
  deleted_volumes = deleted.loc[deleted['id'].isin(created_deleted_ids), 'volume'].reset_index(drop=True)
    
  # Get volumes from the 'created' DataFrame with matched IDs and reset the index
  created_volumes = created.loc[created['id'].isin(created_deleted_ids), 'volume'].reset_index(drop=True)
    
  # Compare volumes
  volume_matched = deleted_volumes == created_volumes
  flashed_ids = created_deleted_ids[volume_matched]
  forever_ids = created[~created['id'].isin(changed['id']) & ~created['id'].isin(deleted['id'])]['id'].reset_index(drop=True)
  ...
  events.loc[events['id'].isin(forever_ids) | events['id'].isin(maker_ids), 'type'] = 'resting-limit'

  # Market limit orders
  ml_ids = taker_ids[np.isin(taker_ids, events[events['event.id'].isin(trades['maker.event.id'])]['id'].unique())]
  ml_ids = ml_ids[~np.isin(ml_ids, pacman_ids)]
  events.loc[events['id'].isin(ml_ids), 'type'] = 'market-limit'

  # Market orders
  mo_ids = taker_ids[~np.isin(taker_ids, events[events['event.id'].isin(trades['maker.event.id'])]['id'].unique())]
