...
def match_trades(events: pd.DataFrame) -> pd.DataFrame:
  ...
  """
  matching_bids = events[(events['direction'] == 'bid') & ~pd.isna(events['matching.event'])].sort_values(by='event.id')
  matching_asks = events[(events['direction'] == 'ask') & ~pd.isna(events['matching.event'])].sort_values(by='matching.event')
  ...
  assert all(matching_bids['event.id'].values == matching_asks['matching.event'].values)
  ...
  bid_maker = (bid_exchange_ts < ask_exchange_ts) | ((bid_exchange_ts == ask_exchange_ts) & (matching_bids['id'] < matching_asks['id']))

  timestamp = np.where(matching_bids['timestamp'] <= matching_asks['timestamp'], 
  ...
  direction = pd.Categorical(np.where(bid_maker, 'sell', 'buy'), categories=['buy', 'sell'], ordered=True).astype(str)
  ...
  combined = pd.DataFrame({'timestamp': timestamp, 'price': price, 'volume': volume,'direction': direction,
                           'maker.event.id': maker_event_id, 'taker.event.id': taker_event_id, 
                           'maker': maker,'taker': taker,
                           'maker_og':maker_og,'taker_og':taker_og})
  ...
  if len(jumps) > 0:
     if jumps[0] == 0:
       jumps = jumps[1:]
     print(f"{combined['timestamp'].iloc[0].strftime('%Y-%m-%d')}: {len(jumps)} jumps > $10 (swapping makers with takers)")
     for i in jumps:
       ...
       if abs(this_jump['price'] - prev_jump['price']) > 10:
         ...
         taker_price = events.loc[events['event.id'] == taker_event_id, 'price'].iloc[0]
         taker_dir = 'sell' if this_jump['direction'] == 'buy' else 'buy'
         swap = pd.DataFrame({'price': taker_price,
                              'direction': taker_dir, 
                              ...
                              'taker_og': this_jump['maker_og']}, index=[i])
          
         combined.loc[i, ['price', 'direction', 'maker.event.id', 'taker.event.id', 'maker', 'taker','maker_og','taker_og']] = swap.iloc[0]
