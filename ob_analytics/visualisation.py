...
def plot_price_levels(depth, spread=None, trades=None, show_mp=True,
                      show_all_depth=False, col_bias=0.1, start_time=None,
    ...
    if spread is not None:
        spread = spread[(spread['timestamp'] >= start_time) & (spread['timestamp'] <= end_time)]
    ...
    if not show_all_depth:
        ...
        for name, group in groups:
            ...
            if len(timestamps) == 2 and timestamps.iloc[0] == start_time and timestamps.iloc[1] == end_time:
...
def plot_price_levels_faster(depth, spread=None, trades=None, show_mp=True, 
    ...
    if price_by is None:
        price_by = 10 ** round(np.log10(depth['price'].max() - depth['price'].min()) - 1)
    ...
    if log_10:
        sns.lineplot(data=depth, x='timestamp', y='price', hue='volume', palette=col_pal, ax=ax)
    else:
        sns.lineplot(data=depth, x='timestamp', y='price', hue='volume', palette=col_pal, ax=ax)
    ...
    if spread is not None:
        if show_mp:
            ...
            ax.plot(spread['timestamp'], spread['best.ask.price'], color='#ff0000', linewidth=1.5)
            ax.plot(spread['timestamp'], spread['best.bid.price'], color='#00ff00', linewidth=1.5)
    
    # Plot trades
    if trades is not None:
        ax.scatter(sells['timestamp'], sells['price'], color='#ff0000', s=50, marker='o', edgecolors='white')
        ax.scatter(buys['timestamp'], buys['price'], color='#00ff00', s=50, marker='o', edgecolors='white')
...
def plot_event_map(events, start_time=None, end_time=None, price_from=None,
    ...
    # Plotting
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.scatterplot(data=created, x='timestamp', y='price', hue='direction',
                    size='volume', palette=col_pal, ax=ax, legend=None)
    sns.scatterplot(data=deleted, x='timestamp', y='price', hue='direction',
                    size='volume', palette=col_pal, ax=ax, legend=None, marker='o', edgecolor=None, alpha=0.5)
...
def plot_volume_map(events, action="deleted", event_type=["flashed-limit"], 
                    start_time=None, end_time=None, price_from=None, price_to=None,
    ...
    mask = (events['action'] == action) & events['type'].isin(event_type) & \
    ...
    if log_scale:
        plt.yscale('log')
    sns.scatterplot(data=events, x='timestamp', y='volume', hue='direction', palette=col_pal, size=0.5, marker='o')
    plt.xlabel('Time')
    plt.ylabel('Volume')
...
def plot_current_depth(order_book, volume_scale=1, show_quantiles=True, show_volume=True):
    ...
    combined_price = bids['price'].tolist() + [bids['price'].iloc[-1], asks['price'].iloc[0]] + asks['price'].tolist()
    combined_liquidity = bids['liquidity'].tolist() + [0, 0] + asks['liquidity'].tolist()
    ...
    # Plotting
    plt.figure(figsize=(10, 6))
    sns.lineplot(data=df[df['side'] == 'ask'], x='price', y='liquidity', color='red', drawstyle='steps-post')
    sns.lineplot(data=df[df['side'] == 'bid'], x='price', y='liquidity', color='blue', drawstyle='steps-pre')

    if show_volume:
        sns.barplot(data=df, x='price', y='volume', hue='side', dodge=False, palette=['blue', 'red'], alpha=0.5)
...
def plot_volume_percentiles(depth_summary, start_time=None, end_time=None, 
    ...
    # Plot ask side
    ax = depth_summary.plot(x='timestamp', y=ask_names, kind='area', stacked=True, colormap='autumn_r')
    if perc_line:
        for col in ask_names:
            depth_summary.plot(x='timestamp', y=col, ax=ax, color='black', linewidth=0.1)
    
    # Plot bid side
    depth_summary[bid_names] = -depth_summary[bid_names]  # invert bid side for visualization
    depth_summary.plot(x='timestamp', y=bid_names, kind='area', stacked=True, ax=ax, colormap='winter')
    if perc_line:
...
def plot_events_histogram(events, start_time=None, end_time=None, val="volume", bw=None):
    ...
    # Plotting
    plt.figure(figsize=(10, 6))
    sns.histplot(data=events, x=val, hue="direction", bins=bw, 
